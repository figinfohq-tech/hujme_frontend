...
const ChoosePaymentOption = () => {
  ...
  const minPartialAmount = Math.ceil(totalAmount * 0.2); // Minimum 20% of total
  // total calculation
         ...
         <Card>
           ...
           <CardContent className="space-y-6">
             ...
             <div className="p-4 bg-accent rounded-lg">
               ...
               {/* <p className="text-sm text-muted-foreground">{travelers.length} traveler(s)</p> */}
               <p className="font-bold text-xl mt-2">Total: ₹{totalAmount.toLocaleString()}</p>
             </div>
             ...
             <div className="space-y-4">
               ...
               <Card 
                 className={cn(
                   ...
                   paymentType === 'full' && "border-primary ring-2 ring-primary/20"
                 )}
               ...
               <Card 
                 className={cn(
                   "cursor-pointer transition-all",
                   paymentType === 'partial' && "border-primary ring-2 ring-primary/20"
                 )}
               ...
               >
                 <CardContent className="p-4">
                   <div className="space-y-3">
                     <div className="flex items-start justify-between">
                       <div className="flex items-start gap-3">
                         ...
                         <div>
                           ...
                           <p className="text-sm text-muted-foreground">
                             Pay a portion now, rest later (minimum ₹{minPartialAmount.toLocaleString()})
                           </p>
                         </div>
                       </div>
                     </div>
                     ...
                     {paymentType === 'partial' && (
                       <div className="pl-7">
                         <Label htmlFor="partial-amount" className="text-sm">Enter Amount</Label>
                         <Input
                           id="partial-amount"
                           type="number"
                         ...
                         {partialAmount > 0 && (
                           <p className="text-xs text-muted-foreground mt-1">
                             Remaining: ₹{(totalAmount - partialAmount).toLocaleString()}
                           </p>
                         )}
                       </div>
                     )}
             ...
             <div className="flex gap-3">
               ...
               <Button
                //  onClick={handlePaymentOptionSubmit}
                 disabled={!paymentType || (paymentType === 'partial' && partialAmount < minPartialAmount)}
                 className="flex-1"
               >
                 Proceed to Payment
               </Button>
