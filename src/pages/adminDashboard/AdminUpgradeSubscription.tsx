...
const AdminUpgradeSubscription = () => {
...
const { agent:agentData } = location.state || {};
  ...
  const fetchAgentSubscription = async () => {
    try {
      const response = await axios.get(
        `${baseURL}agent-subscriptions/byAgent/${agentData?.agentId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleConfirmSubscription = async () => {
    if (!selectedTier || !selectedTier.subscriptionId || !agentData?.agentId) {
...
export default AdminUpgradeSubscription;
