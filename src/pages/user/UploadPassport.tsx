...
export const UploadPassport = () => {
  ...
  useEffect(() => {
    if (booking?.bookingId) {
      setSelectedBookingId(String(booking?.bookingId));
    }

    if (travelerIdFromState) {
      setSelectedPilgrim(String(travelerIdFromState));
    }
  }, [booking?.bookingId, travelerIdFromState]);
  ...
  const handleReplaceDocument = async (doc: Document, file: File) => {
    try {
      ...
      const payload = {
        ...
        travelerId: Number(doc.pilgrimId),
        bookingId: Number(selectedBookingId),
        documentType: "Passport",
        fileName: file.name,
        filePath: `/documents/${userId}/${selectedBookingId}/${selectedPilgrim}/${"Passport"}/${file.name}`,
  ...
  const handleFileUpload = async (travelerId: number, file: File) => {
    if (!selectedBookingId) {
    ...
    setUploadingId(String(travelerId));
    ...
    try {
      ...
      formData.append("travelerId", String(travelerId)); // ✅ CHANGE HERE
  ...
  const allDocumentsUploaded =
    ...
    travelers.every((traveler) =>
      documents.some(
        (doc) =>
          String(doc.pilgrimId) === String(traveler.travelerId) &&
  ...
  return (
    <div className="space-y-8 p-4">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-primary">Upload Possports</h1>
      </div>
      ...
      {viewMode === "upload" && (
        <>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            {travelers.map((docType) => {
              const uploadedDoc = documents.find(
                (d) =>
                  String(d.pilgrimId) === String(docType.travelerId) &&
                  String(d.bookingId) === String(selectedBookingId),
              );
              const isUploading = uploadingId === docType.travelerId;

              return (
                <Card key={docType.travelerId} className="relative">
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div>
                        <CardTitle className="text-lg flex items-center gap-2">
                          <User2 className="h-5 w-5" />
                          {docType.firstName + " " + docType.lastName}
                        </CardTitle>
                        <CardTitle className="text-lg flex items-center gap-2">
                      ...
                      <div className="space-y-4">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            input.onchange = (e) => {
                              const target =
                                e.currentTarget as HTMLInputElement; // ✅ FIX
                      ...
                      <div className="space-y-4">
                        <div className="border-2 border-dashed border-border rounded-lg p-8 text-center">
                          ...
                          <div className="space-y-2">
                            <h4 className="font-medium">Upload Passport</h4>
                            <p className="text-sm text-muted-foreground">
                        ...
                        <div className="space-y-2">
                          <Label htmlFor={`file-${docType.travelerId}`}>
                            Select File
                          </Label>
                          <input
                            id={`file-${docType.travelerId}`}
                            type="file"
                            ...
                            onChange={(e) => {
                              ...
                              if (file)
                                handleFileUpload(docType.travelerId, file);
                            }}
                          />
                        </div>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            document
                              .getElementById(`file-${docType.travelerId}`)
                              ?.click();
                          }}
                        >
                          {isUploading ? (
